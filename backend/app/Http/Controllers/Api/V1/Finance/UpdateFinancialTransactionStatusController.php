...
use App\Http\Requests\Finance\UpdateFinancialTransactionStatusRequest;
use App\Http\Resources\FinancialTransactionResource;
use App\Models\FinancialTransaction;
use App\Services\FinancialTransactionService;
use Illuminate\Http\JsonResponse;

class UpdateFinancialTransactionStatusController extends Controller
{
    public function __construct(private readonly FinancialTransactionService $financialTransactionService) {}

    public function __invoke(UpdateFinancialTransactionStatusRequest $request, FinancialTransaction $financialTransaction): JsonResponse
    {
        $transaction = $this->financialTransactionService->updateStatus(
            $financialTransaction,
            FinancialTransactionStatusEnum::from($request->validated('status')),
        );
