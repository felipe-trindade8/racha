...
it('generates one open income charge per active player for the month', function (): void {
    $active = Player::factory()->count(3)->create();

    $transactions = $this->service->generateMonthlyPayments('2026-06', '50.00');
    ...
    foreach ($active as $player) {
        ...
        expect($charge->type)->toBe(FinancialTransactionTypeEnum::Income)
            ...
            ->and($charge->date->toDateString())->toBe('2026-06-01')
...
it('does not duplicate charges when re-run for the same month', function (): void {
    ...
    $this->service->generateMonthlyPayments('2026-06', '50.00');
    $this->service->generateMonthlyPayments('2026-06', '50.00');
...
it('charges separately for different months', function (): void {
    ...
    $this->service->generateMonthlyPayments('2026-06', '50.00');
    $this->service->generateMonthlyPayments('2026-07', '50.00');
